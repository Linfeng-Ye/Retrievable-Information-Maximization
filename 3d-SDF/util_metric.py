...
def mesh_metrics(pred_mesh, gt_mesh, n_surface_samples, fscore_tau):
    ...
    # nc_lit/nae_lit reproduce this codebase lineage's original eval convention
    # (flat per-triangle face normals, and `nae` averaging the g2p angle with the
    # *raw* g2p cosine rather than the p2g angle -- see ~/NeuRBF/util_metric.py's
    # mesh_metrics(), left unmodified there on purpose). That quirky formula is
    # what NeuRBF's and this codebase lineage's published baseline numbers use,
    # so these are the fields to cite against other papers' tables built on the
    # same eval code.
    normals_p2g_flat = gt_normals_flat[indices_p2g]
    nc_p2g_flat = np.abs(np.sum(normals_p2g_flat * pred_normals_flat, axis=1))
    normals_g2p_flat = pred_normals_flat[indices_g2p]
    nc_g2p_flat = np.abs(np.sum(normals_g2p_flat * gt_normals_flat, axis=1))
    out['nc_lit'] = 0.5 * (np.mean(nc_p2g_flat) + np.mean(nc_g2p_flat))
    nc_g2p_flat_deg = np.degrees(np.arccos(nc_g2p_flat))
    out['nae_lit'] = 0.5 * (np.mean(nc_g2p_flat_deg) + np.mean(nc_g2p_flat))
