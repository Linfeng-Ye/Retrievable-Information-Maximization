...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        gate_weight = float(getattr(self.hparams, 'rim_gate_sparsity_weight', 0.0))
        if gate_weight > 0.0 and hasattr(self.model, 'gate_regularization'):
            loss = loss + gate_weight * self.model.gate_regularization()
    ...
    def train(self, train_loader, valid_loader, max_steps):
        if self.local_rank == 0:
            pbar = tqdm.tqdm(total=max_steps, bar_format='{desc} {percentage:3.0f}% {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]',file=sys.stderr)
        ...
        for data in loader:
            ...
            if self.local_rank == 0:
                ...
                if self.hparams.record_training:
                    ...
                    if (self.global_step % 5000 == 0 or self.global_step == self.max_steps):
                        save_dir = os.path.join(self.workspace, 'results', self.name)
                        save_fn = os.path.join(save_dir, f'step_{str(self.global_step).zfill(6)}')
                        iio.imwrite(f'{save_fn}.png', (pred_latest.clip(self.hparams.vmin, self.hparams.vmax).detach().cpu().numpy() * 255).astype(np.uint8))
                ...
                if self.log_train and ((self.global_step - 1) % 10 == 0 or self.local_step == 1 or self.global_step == self.max_steps):
                    ...
                    gate_stats = self.model.get_gate_stats() if hasattr(self.model, 'get_gate_stats') else None
                    if gate_stats is not None:
                        if self.use_tensorboardX:
                            for k, v in gate_stats.items():
                                if isinstance(v, (int, float)):
                                    self.writer.add_scalar(f"train/{k}", v, self.global_step)
                        metrics_str += f", gate_mean={gate_stats['gate_mean']:.4f}, gate>0.5={gate_stats['gate_frac_gt_0p5']:.4f}"
    ...
    def evaluate_points(self, points_dict): # Only used in SDF task
        self.model.eval()
        self.model.use_train_knn = False
        metrics = {}
