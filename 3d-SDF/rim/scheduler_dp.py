...
from .common3d import q_survival, validate_partition


KappaFn3D = Callable[[int, int], torch.Tensor]
...
def solve_scheduler_dp_fixed_gates(
    ...
    if B != int(A64.shape[0]):
        raise ValueError(f"Gate cube count {B} does not match A cubes {int(A64.shape[0])}")
    M = int(A64.shape[1])
    if M < L:
        raise ValueError(f"Need at least L={L} bands, got M={M}")

    score = torch.full((L, M, M), -float("inf"), dtype=torch.float64, device=A64.device)
    starts = torch.arange(M, device=A64.device)
    valid_intervals = starts[:, None] <= starts[None, :]
    for ell in range(L):
